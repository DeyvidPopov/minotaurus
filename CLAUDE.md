...
- **Public user shape is `toPublicUser` (`auth.controller.ts`) — the single serializer for *every* auth response** (login / register / me / updateMe). Add a user field there once and it propagates everywhere; never hand-build a user object in another response.
...
- **The error handler never leaks internal messages.** `errorHandler` (`middleware/error.ts`) has exactly two branches: an `HttpError` returns its `status`/`code`/`message` verbatim (these are deliberately user-facing — `UNAUTHORIZED`, `NOT_FOUND`, `VALIDATION_ERROR`, `RELATION_EXISTS`, the AI taxonomy, …); **anything else returns a fixed `500 INTERNAL_ERROR` with the generic message `"Internal server error"` — never `err.message`, a stack, or DB/Prisma text.** Full diagnostics (timestamp, error name, message, method, request path, stack) go to `console.error` server-side only. Don't pipe `err.message` (or `details`) from an unexpected error into the response to "help debugging" — that reopens an information-disclosure hole; throw a deliberate `HttpError` if a message is meant to be user-facing. `middleware/error.test.ts` pins this (generic throw / rejected promise ⇒ `"Internal server error"`, and asserts the raw internal text never appears in the body).
