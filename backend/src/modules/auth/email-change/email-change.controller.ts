...
export async function requestChange(req: AuthedRequest, res: Response) {
...
export async function verifyChange(req: AuthedRequest, res: Response) {
  const uid = req.user?.userId;
  if (!uid) return fail(res, 401, "UNAUTHORIZED", "User not found");
...
export async function resendChange(req: AuthedRequest, res: Response) {
  const uid = req.user?.userId;
  if (!uid) return fail(res, 401, "UNAUTHORIZED", "User not found");
